...
class VolumeBars:
    ...
    def load_data(self):
        if self.config.objective_pretrain:
            ...
            self.config.splits['train'] = (
                self.df['Datetime'].min(), self.df['Datetime'].max())
        else:
    ...
    def load_data_for_pretrain(self):
        ...
        for kind, include in data_dict.items():
            if include:
                source_path = self.config.pretrain_config['data_dir'] + \
                    f'/{kind}_data.pqt'
                df = pd.read_parquet(source_path)
                df.reset_index(inplace=True, drop=False)
                df.rename(
                    columns={'Date': 'Datetime', 'index': 'Datetime', 'ticker': 'tic'}, inplace=True)
    ...
    def load_vol_threshold_for_pretrain(self):
        ...
        for kind in ['daily', 'hourly']:
            with open(self.config.data_dir + f'/pretrain_{kind}_volume_thresholds.pkl', 'rb') as f:
                ...
                dict_thresh = {
                    f'{kind[0]}_{tic}': v for tic, v in dict_thresh.items()}
    ...
    def make_one_ticker(self, ticker):
        ...
        tick_df = self.df[self.df['tic'] == ticker][[
            'Datetime'] + self.config.features]
        ...
        df.sort_values(
            by='Datetime', ascending=self.config.prep_for_train, inplace=True)
        ...
        for _, row in df.iterrows():
            ...
            if current_volume >= volume_threshold:
                ...
                dict_chg = {f'delta_{idx}': (
                    (r['Close']/_close) - 1) for idx, (_, r) in enumerate(D.iterrows(), start=1)}
    ...
    def make(self):
        dfs = []
        que = tqdm(self.config.tickers, total=len(
            self.config.tickers), desc='Preprocessing')
        for tick in que:
...
class MyDataset(Dataset):
    ...
    def prepare_data(self):
        ...
        for ticker in self.config.tickers:
            ...
            gadf = GramianAngularField(
                image_size=window, method='difference', sample_range=(0, 1))
            for i in range(len(sub)-window-1, -1, -step_size):
                patch = sub[momentum_targets][i:i+window]
                if len(patch) == window:
                    gadf_image = [gadf.transform(
                        [sub[c][i:i+window]]) for c in cols]
    ...
    def __getitem__(self, idx):
        ...
        price_chg = torch.tensor(
            sample['price_change_targets'], dtype=torch.float32)

        return (ticker, start, image, moment, price_chg)

    def make_transform(self, augment=False):
        transform = transforms.Compose([
            # convert tiff frame to PIL Image
...
class LstmDataset(Dataset):
            ...
            for i in range(len(sub)-window-self.n_forward, -1, -step_size):
                ...
                if len(patch) == window:
                    try:
                        ...
                        moment = [momentum_values[i + window + f - 1]
                                  for f in [1, 2, 3, 4, 5, 6, 7]]
                        # price_change_values[i + window + self.n_forward - 1]
                        price_changes = 0
                        data.append({
                            'ticker': ticker,
                            'start': str(sub['start'].values[i + window]),
    ...
    def __getitem__(self, idx):
        ...
        image = torch.tensor(
            sample['inputs'], dtype=torch.float32).permute(1, 0)
        moment = torch.tensor(sample['momentum_targets'], dtype=torch.float32)
        # torch.tensor(sample['price_change_targets'], dtype = torch.float16)
        price_chg = 0
...
def get_dataloaders(config, fold=1):
    ...
    if config.objective_pretrain:
        ...
        sub_train = train[train['ticker'].isin(
            sorted(train['ticker'].unique().tolist())[:n_train_tics])]
        valid = valid[valid['ticker'].isin(
            sorted(valid['ticker'].unique().tolist())[:n_valid_tics])]
